...
def Rückweisungsverfahren(function, Anzahl_zufallszahlen, x, y):

    verteilung_x = []
    verteilung_y = []

    for i in range(Anzahl_zufallszahlen):
        if y[i] <= function(x[i]):
...
Anzahl_zufallszahlen = int(4.39994e5)
...
random_y = np.random.uniform(y_min, y_cut, Anzahl_zufallszahlen)

# Zufallszahlen der x-Achse der gegebenen Verteilungsfunktion
random_x = np.sort(np.random.uniform(x_min, x_cut, Anzahl_zufallszahlen))

verteilung_x, verteilung_y = Rückweisungsverfahren(planck_verteilung, Anzahl_zufallszahlen, random_x, random_y)
...
def transformation_rückweisung(function, Anzahl, x_s, x_min, x_max, y_min, y_max):
    buffer_x = np.sort(np.random.uniform(x_min, x_max, Anzahl))
    buffer_y = np.random.uniform(y_min, y_max, Anzahl)
    index = len(buffer_x)
    for n in range(len(buffer_x)):
        if buffer_x[n] > x_s:
            index  = n
            break
    buffer_0to1 = np.random.uniform(0, 1, len(buffer_x[index:]))        
    buffer_x[index:] = transformation(buffer_0to1)
    buffer_y[index:] = majorante_planck(transformation(buffer_0to1)) * np.flip(buffer_0to1)
    return Rückweisungsverfahren(function, Anzahl, buffer_x, buffer_y)
...
# Erzeugen der Zufallszahlen gemäß der Majorantenfnktion
